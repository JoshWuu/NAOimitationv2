#server that receives the angles from the client and sends them to the robot
import socket
import sys
import pickle
import math
...
def sendrobot(anglelist, robotIP="192.168.171.141", PORT=9559):
    try:
        ...
        names = ["RShoulderPitch", "RShoulderRoll", "RElbowRoll", "RElbowYaw", "LShoulderPitch", "LShoulderRoll", "LElbowRoll", "LElbowYaw", "HeadYaw", "HeadPitch"]
        ...
        angleLists = [[math.radians(anglelist[len(anglelist) - n])],
                        ...
                        [math.radians(anglelist[len(anglelist) - n + 4])],
                        [math.radians(anglelist[len(anglelist) - n + 5])],
                        [math.radians(anglelist[len(anglelist) - n + 6])],
                        [math.radians(anglelist[len(anglelist) - n + 7])],
                        [math.radians(anglelist[len(anglelist) - n + 8])],
                        [math.radians(anglelist[len(anglelist) - n + 9])]
                        ]
...
def ConnectionServer():
    ...
    while temp == 0:
        ...
        listAngles.append(frames_list[4])#LshoulderPitch
        listAngles.append(frames_list[5])#LshoulderRoll
        listAngles.append(frames_list[6])#LElbowRoll
        listAngles.append(frames_list[7])#LElbowYaw
        listAngles.append(frames_list[8])#HeadYaw
        listAngles.append(frames_list[9])#HeadPitch
        # listAngles.append(frames_list[10])#StandingCrouching
